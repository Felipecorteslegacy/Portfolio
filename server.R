setwd("C:/Users/TheCO/OneDrive/Escritorio/R/Shiny Final/App")
...
general = reactive({        
  ...
  output$PIET = renderPlotly({
    general()
      ...
      fig <- fig %>% add_pie(data = wwpt, labels = ~Assets, values = ~Values,
                             name = "Weights TP", domain = list(row = 0, column = 0))
  ...
  output$PerBL = renderPlotly({
    ...
    ggblp = ggplot(data = vport2) + geom_line(aes(x = seq(1, 153, 1) , y = Sharpe, color = "Sharpe"), size = 1.2) + geom_line(data = vport2, aes(x = seq(1, 153, 1), y = Black_Litterman, color = "Black_Litterman"))
